rm( list = ls () )
setwd("~/Uni/MSc/Bees/nAChRs/02_dexseq_visualisation/tmp")
```
...
for (caste in castes){
  countfiles_tmp <- grep(caste, countfiles, value = TRUE)
  sampleInfo_tmp <- subset(sampleInfo, Caste == caste)
  print(caste)
  print(countfiles_tmp)
  print(sampleInfo_tmp)
  
  #create DEXSeq object
  dxd <- DEXSeqDataSetFromHTSeq(countfiles_tmp, sampleData = sampleInfo_tmp,
  ...
  output_directory <-  paste("results/", caste,
                             "_differential_exon_raw_all/", sep = "")
  dir.create(path = output_directory,
             recursive = TRUE)
  ## Write results to file:
  write.table(dexseq_object_results_df,
      file = paste(output_directory, "dexseq_object_results_all_1_",
                   caste, ".txt",
                   sep = ""),
      row.names = FALSE,
      col.names = TRUE,
  ...
  pdf(paste("plot_MA_1_", caste, ".pdf", sep = ""))
  ...
  pdf(paste("plot_1_", caste, ".pdf", sep = ""))
